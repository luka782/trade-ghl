...
- 信号在 T 日收盘后形成，交易按 T+1 日开盘价加滑点模拟；新持仓从该开盘时点
  开始贡献收益。股票买入后最早只能在下一交易日开盘卖出。
...
- 单标择时采用“信号、仓位、订单、成交”分层，不依赖 vn.py，也未复制 ABu 源码。
- 新增 Donchian 突破 + ATR、双均线趋势 + ATR，并保留原趋势、均值反转、
  RSI/布林带、智能双评分和新旧综合趋势反转作为同口径对照。
- Donchian 上下轨使用 `shift(1)`，只读取 T-1 及更早行情；所有日线止损都由
  T 日收盘确认并在 T+1 开盘执行，不假设能按当日最高/最低价成交。
- 仓位支持全额、固定比例和 ATR 风险预算。ATR 模式按单笔账户风险计算整手数量，
  再受最大仓位、现金、最低成交额和 100 股整数手约束。
- 结果包含入场条件漏斗、市场暴露率、ATR/止损线、风险预算、订单原因和证据充分性。
- Walk-Forward 固定共同最近 3 年及最后 12 个完整月锁定样本外；相同预注册请求
  再次提交时复用已保存结果，避免重复查看锁定 OOS 后继续调参。
...
- 没有逐笔委托簿时，开盘价恰好处于涨跌停价会被保守地视为无法成交。
- 上市/重新上市初期无涨跌停窗口尚未建模；组合选股引擎的会计持仓仍允许
  复权份额，单标择时买入则执行 100 股整数手约束。
